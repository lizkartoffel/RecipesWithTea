...
export default function App() {
  ...
  const loadData = async () => {
    try {
      ...
      const transformedRecipes = recipesData.map((recipe: any) => ({
        ...recipe,
        author: recipe.user_id ? `User ${recipe.user_id}` : "Anonymous",
        rating: 0, // Will be calculated from reviews
        reviewCount: 0,
        cookTime: `${recipe.cook_time || 0} min`,
        prepTime: `${recipe.prep_time || 0} min`,
        totalTime: `${(recipe.prep_time || 0) + (recipe.cook_time || 0)} min`,
        servings: recipe.servings || 4,
        image: recipe.image_url || heroImage,
        dietary: [], // You'll need to add this to backend
        cuisine: "Various", // Map cuisine_id to name
        isFavorite: false,
        ingredients: [],
        instructions: []
      }));
  ...
  const handleViewFullRecipe = (recipe: any) => {
    const recipeContent = `
      ...
      <body>
        ...
        <div class="recipe-meta">
          <div class="meta-item"><strong>Prep Time:</strong> ${recipe.prepTime}</div>
          <div class="meta-item"><strong>Cook Time:</strong> ${recipe.cookTime}</div>
          <div class="meta-item"><strong>Total Time:</strong> ${recipe.totalTime}</div>
  ...
  const handleUploadRecipe = async (recipeData: any) => {
    try {
  ...
  const handleAddReview = async (recipeId: number, rating: number, comment: string) => {
    try {
    ...
    return (
      <div className="min-h-screen bg-[#f5f2e8] flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-2xl font-bold text-[#2d3e1f] mb-2">Loading...</div>
          <p className="text-[#2d3e1f]/70">Brewing your recipes...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex-1">
            ...
            {filteredRecipes.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-[#2d3e1f] opacity-70 text-lg mb-4">
                  {recipes.length === 0 
                ...
                {recipes.length === 0 && isLoggedIn && (
                  <Button
                    onClick={() => setIsUploadModalOpen(true)}
                    className="mt-4 bg-[#4a6741] text-white hover:bg-[#2d3e1f] shadow-md hover:shadow-lg transition-all duration-300"
                  >
